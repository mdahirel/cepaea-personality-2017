...
```{r load-packages}
######all packages up to date as of 19 Feb 2020
library(here)
library(arm)
...
N_warmup <- 5000 ## default number is 1000
...
ggplot()+
  ...
  scale_x_continuous(name = "Test temperature (°C)", limits = c(14, 26),breaks=c(15,20,25)) +
  theme_cowplot()+
  ggtitle("C")
...
### application of Johnson 2014 MEE (doi 10.1111/2041-210X.12225) to get the total random variance of one trait at one obs level (ie including temperature random effect)
...
### to convert latent variances to the observed scale and obtain partial VF variances (VFstate) 
### we follow de Villemereuil et al 2016 (doi: 10.1534/genetics.115.186536) and 2018 (10.1111/jeb.13232)
### and use tools in their QGglmm package
QGlognormal<-list(
...
##tested and checked; does give _exact_ observed scales and variances for 'simple' lognormal (no fixed effect, no random effect, only residual var)

array_fpt_latent <- varcomps_fpt_latent %>% select(beta0:VP) %>% array_tree(.,1) 

varcomps_fpt_obs=tibble(beta0=array_fpt_latent %>%
                          map(.,~QGicc(mu=.["beta0"],var.comp=.["VF"],var.p=.["VP"],
                                       custom.model = QGlognormal,verbose=F)$mean.obs) %>% as_vector(),
                        VF=array_fpt_latent %>%
                          map(.,~QGicc(mu=.["beta0"],var.comp=.["VF"],var.p=.["VP"],
  ...
  select(beta0:VBint,VD,VP) %>% ##just to keep in the same logical order as in the latent tab
...
varcomps_bold_obs=tibble(beta0=array_bold_latent %>%
                          map(.,~QGicc(mu=.["beta0"],var.comp=.["VF"],var.p=.["VP"],
                                       custom.model = QGlognormal,verbose=F)$mean.obs) %>% as_vector(),
...
vc_complete=rbind(varcomps_fpt_obs,varcomps_bold_obs,varcomps_fpt_latent,varcomps_bold_latent)
```
